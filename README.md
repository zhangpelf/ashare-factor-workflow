# A-Share Factor Workflow — Claude Code Skills Pack

> A complete Claude Code skills pack for systematic quantitative factor research on China A-shares. From literature survey to final report — with adversarial review, multi-method mining, and 8-module evaluation.
...
**Companion repo**: [ashare-factor-mining](https://github.com/zhangpelf/ashare-factor-mining) — Python factor computation, testing pipeline, and visualization engine.

---

## Overview

This skills pack defines a **structured, auditable workflow** for quantitative factor research. It transforms the ad-hoc process of factor mining into a repeatable pipeline with clear gates, standardized evaluation, and traceable artifacts.

```
Literature ──► Mining ──► Testing ──► Review ──► Report
   (G001)      (G002)      (G003)      (G004)     (G005/G006)
```

Each stage produces artifacts in `paper_research/` (literature) or `output/` (data/code/reports) for full audit trail.
...
- **6-dimension evaluation**: IC, IR, Sharpe, Fama-MacBeth t-stat, long-short annualized return, IC positive ratio
- **Literature cross-validation**: compare A-share results against academic paper expectations
- **Adversarial review loop**: cross-model critique (Claude ↔ GPT) for code and result integrity
- **Full audit trail**: every step leaves artifacts for manual review
